...
const FarmCards: React.FC = () => {
  ...
  const iptIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'IPT',
  )

  const iptPrice =
    iptIndex >= 0 && stakedValue[iptIndex]
      ? stakedValue[iptIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2372500)
  const IPT_PER_BLOCK = new BigNumber(70)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [ipt, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'IPT' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
